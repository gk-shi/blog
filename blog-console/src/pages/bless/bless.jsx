...
class Bless extends React.Component {
  ...
  deleteBless = async (id) => {
    const ret = await this.props.deleteBless(id)
    if (ret && ret.error) {
      openNotificationWithIcon('error', '删除失败！', ret.message)
    } else {
  ...
  publish = async (text) => {
    ...
    const ret = await this.props.addBless(bless)
    if (ret && ret.error) {
      openNotificationWithIcon('error', '发布失败！', ret.message)
    } else {
