...
class Create extends React.Component {
  ...
  getOldArticle = async (id) => {
    ...
    if (data.errno === 0) {
      ...
      ;(data.data.markdown || data.data.content) && this.state.$mark.current.setDefaultValue(data.data.markdown || data.data.content)
      data.data.cover && this.state.$cover.current.setDefaultImg(data.data.cover)
    }
  }
