...
import { sendEmailService, updateAdminService, getOneOfDay } from '../../services/request'
...
class Login extends React.Component {
  ...
  resetPassword = async (values) => {
    ...
    try {
      const { data } = await updateAdminService(adminInfo)
      if (data.errno === 0) {
  ...
  sendCode = async () => {
    ...
    try {
      ...
      const { data } = await sendEmailService(email)
  ...
  render () {
    ...
    const { forgot, loading, sendBtnTip, restSeconds, sending, initValue, oneWord } = this.state
    const msgTip = {
