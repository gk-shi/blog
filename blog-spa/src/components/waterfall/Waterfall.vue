  ...
  <div class="vue-waterfall-easy-container" :style="{width: width&&!isMobile ? width+'px' : '', height: parseFloat(height)==height ? height+'px': height }">
    ...
    <div class="vue-waterfall-easy-scroll" ref="scrollEl">
      ...
      <div class="vue-waterfall-easy" :style="isMobile? '' :{width: colWidth*cols+'px',left:'50%', marginLeft: -1*colWidth*cols/2 +'px'}">
        <div class="img-box" v-for="(v, i) in imgsArr_c" :key="'n' + i" :class="[cardAnimationClass, {__err__: v._error}]" :style="{padding: (isMobile ? mobileGap : gap)/2+'px', width: isMobile ? '' : colWidth+'px'}">
          <alink class="img-inner-box" :is="isRouterLink  && linkRange=='card' ? 'router-link' : 'alink'" :data-index="i" :to="linkRange=='card' ? v[hrefKey] : false" @mouseover.native="bigOrSmall(i)" @mouseout.native="bigOrSmall(i)">
            <alink class="img-wraper" v-if="v[srcKey]" :is="isRouterLink && linkRange=='img' ? 'router-link' :'alink'" :to="linkRange=='img' ? v[hrefKey] : false " :style="{width:imgWidth_c + 'px',height:v._height ? v._height+'px':false}">
              <!--  :style="{width:imgWidth_c + 'px',height:v._height ? v._height+'px':false}" -->
              <img :src="v[srcKey]" alt="" :class="!scaleArr[i] ? 'scale-d' : 'scale-img'">
            </alink>
            <slot :index="i" :value="v"></slot>
          </alink>
        </div>
        <div class="over" v-if="over" ref="over">
          <slot name="waterfall-over">哈，被你看光了~</slot>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    imgWidth_c () { // 对于移动端重新计算图片宽度`
      return this.isMobile ? window.innerWidth - this.mobileGap : this.imgWidth
    },
    hasLoadingSlot () {
  ...
  watch: {
    ...
    imgsArr (newV, oldV) {
      this.scaleArr = new Array(newV.length).fill(false)
  ...
  methods: {
    bigOrSmall (i) {
      this.$set(this.scaleArr, i, !this.scaleArr[i])
    },
    // ==1== 预加载
    ...
    waterfall () {
      ...
      let top, left, height, colWidth = this.isMobile ? this.imgBoxEls[0].offsetWidth : this.colWidth
    ...
    // ==7== 下拉事件
    pullDown () {
      let scrollEl = this.$el.querySelector('.vue-waterfall-easy-scroll')
      let startY
...
.scale-d {
  animation: scaleD 80ms linear forwards;
}
...
.scale-img {
  animation: scaleImg 80ms linear forwards;
}
