...
export async function list (ctx: Context): Promise<any> {
  ...
  try {
    ...
    responseHandle({ ctx, errno: -1, errmsg: '获取留言失败，等等再试吧~', error })
  }
}

// 新增留言
export async function create (ctx: Context): Promise<any> {
  const body: IBless = ctx.request.body
  ...
  try {
    ...
    responseHandle({ ctx, errno: -1, errmsg: '新增留言失败，等等再试吧~', error })
  }
}
...
export async function del (ctx: Context): Promise<any> {
  ...
  try {
    ...
    responseHandle({ ctx, errno: -1, errmsg: '删除留言失败~', error })
  }
}