...
博客服务端采用`koa2`框架搭建，配合的数据库为`MongoDB`，全部使用 `TS` 重构。配置文件为根目录的`admin.js`和`config.js`两个文件。连个文件的说明如下：
...
# 正式编译 (注意：打包仅仅是将 TS 编译成 JS ，服务器部署时需要重新安装生产依赖，并且启动时要自己加入 NODE_ENV: production)
npm run build
```
